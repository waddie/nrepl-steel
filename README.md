...
A pure-Scheme nREPL server that runs inside a Steel runtime.

## Requirements

Steel 0.8.2 (`steel` on PATH) and [Forge](https://github.com/mattwparas/steel) (`forge` on PATH). No build step.
...
# Or directly from the repository (no clone needed):
...
Not implemented in stage one: streaming `out`, `stdin`/`need-input`,
`completions`/`lookup`. `interrupt` is queued-only.
...
```sh
./run-tests.sh             # unit suite (single process): bencode, transport,
                           # evaluator/session, dispatch, serve-loop - 106 assertions
...
test/integration.sh        # acceptance gate: starts a real server and drives it from
                           ...
                           # load-file -> ls-sessions -> interrupt -> close)
```
